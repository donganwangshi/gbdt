...
import org.apache.spark.mllib.tree.configuration.Strategy_new
import org.apache.spark.mllib.tree.configuration.Algo._
import org.apache.spark.mllib.tree.configuration.FeatureType._
import org.apache.spark.mllib.tree.configuration.QuantileStrategy._
import org.apache.spark.mllib.tree.impl._
import org.apache.spark.mllib.tree.impurity.{Impurities_new, Impurity_new}
 ...
 */
@Experimental
class DecisionTree_new (private val strategy: Strategy_new) extends Serializable with Logging {
   ...
   */
  def run(input: RDD[LabeledPoint]): DecisionTreeModel_new = {
  ...
  def train(input: RDD[LabeledPoint]): DecisionTreeModel_new = run(input)
}

object DecisionTree_new extends Serializable with Logging {
   ...
   * @return DecisionTreeModel that can be used for prediction
  */
  def train(input: RDD[LabeledPoint], strategy: Strategy_new): DecisionTreeModel_new = {
    new DecisionTree_new(strategy).run(input)
  }
  ...
  def train(
      ...
      algo: Algo,
      impurity: Impurity_new,
      maxDepth: Int): DecisionTreeModel_new = {
    val strategy = new Strategy_new(algo, impurity, maxDepth)
    new DecisionTree_new(strategy).run(input)
  }
  ...
  def train(
      ...
      impurity: Impurity_new,
      maxDepth: Int,
      numClassesForClassification: Int): DecisionTreeModel_new = {
    val strategy = new Strategy_new(algo, impurity, maxDepth, numClassesForClassification)
    new DecisionTree_new(strategy).run(input)
  }
  ...
  def train(
      ...
      categoricalFeaturesInfo: Map[Int,Int]): DecisionTreeModel_new = {
    val strategy = new Strategy_new(algo, impurity, maxDepth, numClassesForClassification, maxBins,
  ...
  def trainClassifier(
      ...
      impurity: String,
      maxDepth: Int,
      maxBins: Int): DecisionTreeModel_new = {
    val impurityType = Impurities_new.fromString(impurity)
  ...
  private def predictNodeIndex(
    ...
    if (node.isLeaf || node.split.isEmpty) {
      ...
      if (node.leftNode.isEmpty || node.rightNode.isEmpty) {
        ...
        if (splitLeft) {
          Node_new.leftChildIndex(node.id)
        } else {
          Node_new.rightChildIndex(node.id)
        }
      } else {
        if (splitLeft) {
  ...
  private[tree] def findBestSplits(
      input: RDD[BaggedPoint[TreePoint_new]],
      metadata: DecisionTreeMetadata_new,
      topNodes: Array[Node_new],
      nodesForGroup: Map[Int, Array[Node_new]],
      treeToNodeToIndexInfo: Map[Int, Map[Int, NodeIndexInfo]],
      splits: Array[Array[Split]],
      bins: Array[Array[Bin]],
      nodeQueue: mutable.Queue[(Int, Node_new)],
    ...
    Logger.getRootLogger().error("numNodes = " + numNodes)
    Logger.getRootLogger().error("numFeatures = " + metadata.numFeatures)
    Logger.getRootLogger().error("numClasses = " + metadata.numClasses)
    Logger.getRootLogger().error("isMulticlass = " + metadata.isMulticlass)
    Logger.getRootLogger().error("isMulticlassWithCategoricalFeatures = " +
      metadata.isMulticlassWithCategoricalFeatures)
    Logger.getRootLogger().error("using nodeIdCache = " + nodeIdCache.nonEmpty.toString)
    ...
    def nodeBinSeqOp(
        ...
        baggedPoint: BaggedPoint[TreePoint_new]): Unit = {
      if (nodeInfo != null) {
    ...
    def binSeqOp(
        agg: Array[DTStatsAggregator],
        baggedPoint: BaggedPoint[TreePoint_new]): Array[DTStatsAggregator] = {
    ...
    def binSeqOpWithNodeIdCache(
        agg: Array[DTStatsAggregator],
        dataPoint: (BaggedPoint[TreePoint_new], Array[Int])): Array[DTStatsAggregator] = {
      treeToNodeToIndexInfo.foreach { case (treeIndex, nodeIndexToInfo) =>
    ...
    val nodes = new Array[Node_new](numNodes)
    ...
    val nodeToBestSplits = partitionAggregates.reduceByKey((a, b) => a.merge(b))
        .map { case (nodeIndex, aggStats) =>
          ...
          val (split: Split, stats: InformationGainStats_new, predict: Predict) =
    ...
    nodesForGroup.foreach { case (treeIndex, nodesForTree) =>
      nodesForTree.foreach { node =>
        ...
        Logger.getRootLogger().error("best split = " + split)

        // Extract info for this node.  Create children if not leaf.
        val isLeaf = (stats.gain <= 0) || (Node_new.indexToLevel(nodeIndex) == metadata.maxDepth)
        assert(node.id == nodeIndex)
        node.predict = predict
        node.isLeaf = isLeaf
        node.stats = Some(stats)
        node.impurity = stats.impurity
        Logger.getRootLogger().error("Node = " + node)

        if (!isLeaf) {
          node.split = Some(split)
          val childIsLeaf = (Node_new.indexToLevel(nodeIndex) + 1) == metadata.maxDepth
          val leftChildIsLeaf = childIsLeaf || (stats.leftImpurity == 0.0)
          val rightChildIsLeaf = childIsLeaf || (stats.rightImpurity == 0.0)
          node.leftNode = Some(Node_new(Node_new.leftChildIndex(nodeIndex),
            stats.leftPredict, stats.leftImpurity, leftChildIsLeaf))
          node.rightNode = Some(Node_new(Node_new.rightChildIndex(nodeIndex),
          ...
          Logger.getRootLogger().error("leftChildIndex = " + node.leftNode.get.id +
            ", impurity = " + stats.leftImpurity)
          Logger.getRootLogger().error("rightChildIndex = " + node.rightNode.get.id +
  ...
  private def calculateGainForSplit(
      ...
      metadata: DecisionTreeMetadata_new,
      impurity: Double): InformationGainStats_new = {
    ...
    if ((leftCount < metadata.minInstancesPerNode) ||
      ...
      return InformationGainStats_new.invalidInformationGainStats
    }
    ...
    new InformationGainStats_new(gain, impurity, leftImpurity, rightImpurity,
  ...
  private def binsToBestSplit(
      ...
      node: Node_new): (Split, InformationGainStats_new, Predict) = {

    // calculate predict and impurity if current node is top node
    val level = Node_new.indexToLevel(node.id)
    ...
    val (bestSplit, bestSplitStats) =
      ...
      if (binAggregates.metadata.isContinuous(featureIndex)) {
        ...
        Logger.getRootLogger().error("Centroids for categorical variable: " + centroidForCategories.mkString(","))

        // bins sorted by centroids
        val categoriesSortedByCentroid = centroidForCategories.toList.sortBy(_._2)

        Logger.getRootLogger().error("Sorted centroids for categorical variable = " +
  ...
  protected[tree] def findSplitsBins(
      input: RDD[LabeledPoint],
      metadata: DecisionTreeMetadata_new): (Array[Array[Split]], Array[Array[Bin]]) = {
    ...
    val sampledInput = if (hasContinuousFeatures) {
      ...
      Logger.getRootLogger().error("fraction of data used for calculating quantiles = " + fraction)
      ...
      case Sort =>
        ...
        while (featureIndex < numFeatures) {
          if (metadata.isContinuous(featureIndex)) {
            ...
            //Logger.getRootLogger().error(s"featureIndex = $featureIndex, numSplits = $numSplits")
  ...
  private[tree] def findSplitsForContinuousFeature(
    ...
    val splits = {
      ...
      if (possibleSplits <= numSplits) {
        ...
        //Logger.getRootLogger().error("stride = " + stride)
