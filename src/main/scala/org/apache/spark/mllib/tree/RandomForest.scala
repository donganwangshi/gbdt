...
import org.apache.spark.mllib.tree.impl.{BaggedPoint, DecisionTreeMetadata_new, NodeIdCache,TimeTracker, TreePoint_new}
import org.apache.spark.mllib.tree.impurity.Impurities_new
...
private class RandomForest (
    private val strategy: Strategy_new,
  ...
  def run(input: RDD[LabeledPoint]): RandomForestModel = {
    ...
    val metadata =
      DecisionTreeMetadata_new.buildMetadata(retaggedInput, strategy, numTrees, featureSubsetStrategy)
    Logger.getRootLogger().error("algo = " + strategy.algo)
    Logger.getRootLogger().error("numTrees = " + numTrees)
    Logger.getRootLogger().error("seed = " + seed)
    Logger.getRootLogger().error("maxBins = " + metadata.maxBins)
    Logger.getRootLogger().error("featureSubsetStrategy = " + featureSubsetStrategy)
    Logger.getRootLogger().error("numFeaturesPerNode = " + metadata.numFeaturesPerNode)

    // Find the splits and the corresponding bins (interval between the splits) using a sample
    // of the input data.
    timer.start("findSplitsBins")
    val (splits, bins) = DecisionTree_new.findSplitsBins(retaggedInput, metadata)
    timer.stop("findSplitsBins")
    Logger.getRootLogger().error("numBins: feature: number of bins")
    Logger.getRootLogger().error(Range(0, metadata.numFeatures).map { featureIndex =>
        s"\t$featureIndex\t${metadata.numBins(featureIndex)}"
      }.mkString("\n"))

    // Bin feature values (TreePoint representation).
    // Cache input RDD for speedup during multiple passes.
    val treeInput = TreePoint_new.convertToTreeRDD(retaggedInput, bins, metadata)
    ...
    Logger.getRootLogger().error("max memory usage for aggregates = " + maxMemoryUsage + " bytes.")
    ...
    val nodeQueue = new mutable.Queue[(Int, Node_new)]()

    val rng = new scala.util.Random()
    rng.setSeed(seed)

    // Allocate and queue root nodes.
    val topNodes: Array[Node_new] = Array.fill[Node_new](numTrees)(Node_new.emptyNode(nodeIndex = 1))
    ...
    while (nodeQueue.nonEmpty) {
      ...
      DecisionTree_new.findBestSplits(baggedInput, metadata, topNodes, nodesForGroup,
    ...
    val trees = topNodes.map(topNode => new DecisionTreeModel_new(topNode, strategy.algo))
...
object RandomForest extends Serializable with Logging {
  ...
  def trainClassifier(
    ...
    val strategy = new Strategy_new(Classification, impurityType, maxDepth,
  ...
  def trainRegressor(
    ...
    require(strategy.algo == Regression,
      s"RandomForest.trainRegressor given Strategy_new with invalid algo: ${strategy.algo}")
    val rf = new RandomForest(strategy, numTrees, featureSubsetStrategy, seed)
    rf.run(input)
  }
  ...
  def trainRegressor(
      ...
      seed: Int = Utils.random.nextInt()): RandomForestModel = {
    val impurityType = Impurities_new.fromString(impurity)
    val strategy = new Strategy_new(Regression, impurityType, maxDepth,
  ...
  private[tree] def selectNodesToSplit(
      ...
      rng: scala.util.Random): (Map[Int, Array[Node_new]], Map[Int, Map[Int, NodeIndexInfo]]) = {
    // Collect some nodes to split:
    //  nodesForGroup(treeIndex) = nodes to split
    val mutableNodesForGroup = new mutable.HashMap[Int, mutable.ArrayBuffer[Node_new]]()
    ...
    while (nodeQueue.nonEmpty && memUsage < maxMemoryUsage) {
      ...
      if (memUsage + nodeMemUsage <= maxMemoryUsage) {
        nodeQueue.dequeue()
        mutableNodesForGroup.getOrElseUpdate(treeIndex, new mutable.ArrayBuffer[Node_new]()) += node
    ...
    val nodesForGroup: Map[Int, Array[Node_new]] = mutableNodesForGroup.mapValues(_.toArray).toMap
