...
import org.apache.spark.mllib.tree.impurity.{Variance_new, Entropy_new, Gini_new, Impurity_new}
...
class Strategy_new (
    ...
    @BeanProperty var impurity: Impurity_new,
  ...
  def copy: Strategy_new = {
    new Strategy_new(algo, impurity, maxDepth, numClassesForClassification, maxBins,
...
object Strategy_new {
   ...
   */
  def defaultStrategy(algo: String): Strategy_new = algo match {
    case "Classification" =>
      new Strategy_new(algo = Classification, impurity = Gini_new, maxDepth = 3,
        numClassesForClassification = 2)
    case "Regression" =>
      new Strategy_new(algo = Regression, impurity = Variance_new, maxDepth = 3,
