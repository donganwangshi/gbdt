...
private[tree] class DTStatsAggregator(
    val metadata: DecisionTreeMetadata_new,
  ...
  val impurityAggregator: ImpurityAggregator = metadata.impurity match {
    case Gini_new => new GiniAggregator(metadata.numClasses)
    case Entropy_new => new EntropyAggregator(metadata.numClasses)
    case Variance_new => new VarianceAggregator()
