 ...
 */
private[tree] class TreePoint_new(val label: Double, val binnedFeatures: Array[Int])
  extends Serializable {
}

private[tree] object TreePoint_new {
  ...
  def convertToTreeRDD(
      ...
      metadata: DecisionTreeMetadata_new): RDD[TreePoint_new] = {
    ...
    input.map { x =>
      TreePoint_new.labeledPointToTreePoint(x, bins, featureArity, isUnordered)
    }
  }
  ...
  private def labeledPointToTreePoint(
      ...
      isUnordered: Array[Boolean]): TreePoint_new = {
    ...
    new TreePoint_new(labeledPoint.label, arr)
  }
