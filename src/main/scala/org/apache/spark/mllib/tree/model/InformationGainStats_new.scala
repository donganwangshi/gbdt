 ...
 */
@DeveloperApi
class InformationGainStats_new(
    val gain: Double,
    val impurity: Double,
  ...
  override def equals(o: Any) =
    o match {
      case other: InformationGainStats_new => {
        gain == other.gain &&
...
private[tree] object InformationGainStats_new {
  /**
   * An [[org.apache.spark.mllib.tree.model.InformationGainStats]] object to
   * denote that current split doesn't satisfies minimum info gain or
   * minimum number of instances per node.
   */
  val invalidInformationGainStats = new InformationGainStats_new(Double.MinValue, -1.0, -1.0, -1.0,
