...
class Node_new (
    ...
    var leftNode: Option[Node_new],
    var rightNode: Option[Node_new],
    var stats: Option[InformationGainStats_new]) extends Serializable with Logging {
  ...
  @deprecated("build should no longer be used since trees are constructed on-the-fly in training",
    "1.2.0")
  def build(nodes: Array[Node_new]): Unit = {
    logDebug("building node " + id + " at level " + Node_new.indexToLevel(id))
    logDebug("id = " + id + ", split = " + split)
    logDebug("stats = " + stats)
    logDebug("predict = " + predict)
    logDebug("impurity = " + impurity)
    if (!isLeaf) {
      leftNode = Some(nodes(Node_new.leftChildIndex(id)))
      rightNode = Some(nodes(Node_new.rightChildIndex(id)))
   ...
   */
  private[tree] def deepCopy(): Node_new = {
    ...
    new Node_new(id, predict, impurity, isLeaf, split, leftNodeCopy, rightNodeCopy, stats)
  }
...
private[tree] object Node_new {

  /**
   * Return a node with the given node id (but nothing else set).
   */
  def emptyNode(nodeIndex: Int): Node_new = new Node_new(nodeIndex, new Predict(Double.MinValue), -1.0,
  ...
  def apply(
      ...
      predict: Predict,
      impurity: Double,
      isLeaf: Boolean): Node_new = {
    new Node_new(nodeIndex, predict, impurity, isLeaf, None, None, None, None)
  }
   ...
   */
  def getNode(nodeIndex: Int, rootNode: Node_new): Node_new = {
    var tmpNode: Node_new = rootNode
